...
async function fetchForecast(city) {
  try {
    ...
    console.log(data); // Adicione isso para verificar a resposta da API
...
function displayForecast(forecastData) {
  ...
  forecastData.forEach((entry) => {
    // Exibe previsões a cada 3 horas
...
function addMarker(lat, lon, cityName) {
  ...
  marker.bindPopup(`${cityName} - Coordenadas: ${lat.toFixed(4)}, ${lon.toFixed(4)}`).openPopup();
}
...
document.getElementById('getLocationButton').addEventListener('click', () => {
  getUserLocation(); // Chama a função para obter a localização
});
...
function initMap() {
  ...
  // Impede o menu de contexto padrão do navegador
  map.on('contextmenu', (e) => {
    e.originalEvent.preventDefault();
  });

  map.on('click', onMapClick);
  map.on('contextmenu', onMapRightClick); // Adiciona o evento de clique direito
...
// Função de clique no mapa para obter clima pela coordenada ou exibir coordenadas
...
// Função para exibir coordenadas ao clicar com o botão direito
function onMapRightClick(e) {
  const lat = e.latlng.lat;
  const lon = e.latlng.lng;

  // Adiciona um marcador no local clicado com o botão direito
  L.marker([lat, lon]).addTo(map).bindPopup(`Coordenadas: ${lat.toFixed(4)}, ${lon.toFixed(4)}`).openPopup();
}
